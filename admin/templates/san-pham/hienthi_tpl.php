        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <div class="pull-right">
                    <a href="index.php?p=<?=$_GET['p']?>&a=variation" class="btn btn-primary pull-right"><i class="glyphicon glyphicon-plus"></i> Biến thể sản phẩm</a>
                        ...
                        <table class="table table-bordered table-striped table-primary table-hover">
                            ...
                            <tbody id="data-ajax">
                                ...
                                <tr>
                                        ...
                                        <span class="font-size:12px">
                                            <?php 
                                                if($items[$i]['id_loai']>0){
                                                    ...
                                                    echo (!empty($query['ten'])) ? $query['ten']:'';
                                                }
                                            ?>
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <?php if ($items[$i]['khuyen_mai'] > 0 ): ?>
                                            <span class="text-danger"><?= number_format($items[$i]['khuyen_mai'] , 0,',','.')."đ" ?></span><br>
                                            <span style="text-decoration: line-through;"><?= number_format($items[$i]['gia'] , 0,',','.')."đ" ?></span>
                                        <?php else: ?>
                                            <span class="text-danger"><?= number_format($items[$i]['gia'] , 0,',','.')."đ" ?></span>
                                        <?php endif ?>
                                    </td>

                                    <td class="text-center">
                                        <?=($items[$i]['hinh_anh'] <> '') ? "<img src='../img_data/images/".$items[$i]['hinh_anh']."' class='img_object50' >" : ""; ?>
                                    </td>
                                    ...
                                    <td class="text-left" style="line-height: 10px;">
                                        <label style="font-size: 13px; margin-bottom: 2px;">
                                            <input class="chk_box" <?php if($d->checkPermission_edit($id_module)==0){ ?>disabled<?php }?> type="checkbox" onclick="on_check(this,'#_sanpham','tieu_bieu','<?=$items[$i]['id_code']?>')" <?php if($items[$i]['tieu_bieu'] == 1) echo 'checked="checked"'; ?>> Nổi bật
                                        </label><br>
                                        <label style="font-size: 13px; margin-bottom: 2px;">
                                            <input class="chk_box" <?php if($d->checkPermission_edit($id_module)==0){ ?>disabled<?php }?> type="checkbox" onclick="on_check(this,'#_sanpham','sp_moi','<?=$items[$i]['id_code']?>')" <?php if($items[$i]['sp_moi'] == 1) echo 'checked="checked"'; ?>> Mới
                                        </label><br>
                                        <label style="font-size: 13px; margin-bottom: 2px;">
                                            <input class="chk_box" <?php if($d->checkPermission_edit($id_module)==0){ ?>disabled<?php }?> type="checkbox" onclick="on_check(this,'#_sanpham','sp_hot','<?=$items[$i]['id_code']?>')" <?php if($items[$i]['sp_hot'] == 1) echo 'checked="checked"'; ?>> Hot
                                        </label><br>
                                        <label style="font-size: 13px; margin-bottom: 2px;">
                                            <input class="chk_box" <?php if($d->checkPermission_edit($id_module)==0){ ?>disabled<?php }?> type="checkbox" onclick="on_check(this,'#_sanpham','sp_sale','<?=$items[$i]['id_code']?>')" <?php if($items[$i]['sp_sale'] == 1) echo 'checked="checked"'; ?>> Sale
                                        </label><br>
                                        <label style="font-size: 13px; margin-bottom: 2px;">
                                            <input class="chk_box" <?php if($d->checkPermission_edit($id_module)==0){ ?>disabled<?php }?> type="checkbox" onclick="on_check(this,'#_sanpham','sp_top','<?=$items[$i]['id_code']?>')" <?php if($items[$i]['sp_top'] == 1) echo 'checked="checked"'; ?>> Top
                                        </label>
                                    </td>
    ...
    jQuery(document).ready(function($) {
        ...
        onPageClick: function (event, page) {
            $.ajax({
                ...
                type:'POST',
                data: {page: page,totalPages:'<?=$total_page?>', where: where, limit:'<?=$limit?>',search:'<?=$link_search?>',id_module:id_module},
                success: function(data){
                    if(data){
                        $('#data-ajax').html(data);
                    }
