    ...
    if(!isset($_GET['id_loai'])){
        ...
        if($id_code = $d->insert($data0)) {
            ...
            foreach (get_json('lang') as $key => $value) {
                ...
                $data_cate['alias']          =   addslashes($_POST['alias'][$key]);
...
if(isset($_GET['delete']) and $_GET['delete']!=''){
    if($d->checkPermission_dele($id_module)==1){
        $id =  (int)$_GET['delete'];
        $sub_content = $d->o_fet("select * from #_filter_content where id_loai = '".$id."'");
        if(count($sub_content)>0){
            foreach ($sub_content as $key => $value) {
                $d->o_que("delete from cf_parent where id = ".$value['id_code']." ");
            }
            $d->o_que("delete from #_filter_content where id_loai = ".$id." ");
        }
        $d->reset();
        $d->setTable('#_filter');
        ...
        <div class="row">
            <div class="col-sm-8">
                <div class="box box-primary">
                    ...
                    <?php 
                          //c1
                          $danhmuc_home = $d->o_fet("select * from #_filter where lang='".LANG."' order by id ASC"); 
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-primary">
                            ...
                            <tbody>
                                ...
                                $hinhanh = $d->o_fet("select * from #_filter_content where id_loai=".$value['id_code']." and lang ='".LANG."' order by so_thu_tu ASC, id ASC");
                                ?>
                                <tr>
                                    <td class="text-center">
                                        ...
                                        <?php }?>
                                        <a href="index.php?p=filter&a=add&parent=<?=$value['id_code']?>" class="btn btn-xs btn-primary">Thêm tùy chọn</a>
                                    </td>
                                    <td><?=$value['ten']?></td>
                                    <td class="text-center">
                                        <?php if($value['hinh_anh']!=''){ ?>
                                        <img src="../img_data/images/<?=$value['hinh_anh'] ?>" style="max-width:50px">
                                        <?php }?>
                                    </td>
                                    <td class="text-center">
                                        <input <?php if($d->checkPermission_edit($id_module)==0){ ?> disabled <?php }?> class="chk_box" type="checkbox" onclick="on_check(this,'#_filter','hien_thi','<?=$value['id']?>')" <?php if($value['hien_thi'] == 1) echo 'checked="checked"'; ?>>
                                    </td>
                                    <td class="text-center">
                                ...
                                <tr <?=$_GET['parent']==$value['id_code']?'':'style="display: none"'?> class="noi_dun_<?=$value['id']?>">
                                    <td class="text-center">
                                        ...
                                        <input type="number" value="<?=$value2['so_thu_tu']?>" class="a_stt" data-table="#_filter_content" data-col="so_thu_tu" data-id="<?=$value2['id_code']?>" />
                                        <?php }else{?>
                                    ...
                                    <td class="text-center">
                                        <input class="chk_box" <?php if($d->checkPermission_edit($id_module)==0){ ?> disabled <?php }?> type="checkbox" onclick="on_check(this,'#_filter_content','hien_thi','<?=$value2['id_code']?>')" <?php if($value2['hien_thi'] == 1) echo 'checked="checked"'; ?>>
                                    </td>
                                    <td class="text-center">
                    ...
                    <div class="box-body">
                        ...
                        <form method="POST" action=""  enctype="multipart/form-data">
                            <div class="form-group m0 hinh_anh" >
                                ...
                                <span class="box-img2">
                                    <?php if(isset($_GET['id_loai']) and  $_GET['id_loai']!='' ){
                                        $row0 = $d->simple_fetch("select * from #_filter where id_code = ".(int)$_GET['id_loai']." and lang = '".LANG."' ");
                                    ?>
                                    <img onerror="this.src='img/no-image.png';" src="../img_data/images/<?php echo $row0['hinh_anh']?>"  id="review_hinh_anh" alt="NO PHOTO" />
                                    <button class="btn btn-xs btn-danger" type="button" onclick="xoa_img('_category_noidung','hinh_anh', '<?php echo (!isset($_GET['parent']) ? $_GET['id_loai']:$_GET['id']  )?>','')"><i class="fa fa-trash"></i></button>
                                    <?php }else{ ?>
                            ...
                            <div id="myTabContent" class="tab-content">
                                <?php foreach (get_json('lang') as $key => $value) {
                                if(isset($_GET['id_loai'])){
                                    $row = $d->simple_fetch("select * from #_filter where id_code = ".(int)$_GET['id_loai']." and lang = '".$value['code']."' ");
                                }    
                                ?>
                                <div role="tabpanel" class="tab-pane fade in <?php if($key==0){?> active <?php }?>" id="seo_<?=$value['code']?>" aria-labelledby="seo_<?=$value['code']?>">
                                    <div class="form-group">
                                        ...
                                        <div class="row m-5">
                                            <div class="col-sm-8 p5">
                                                <input type="text" value="<?=$row['ten']?>" placeholder="Nhập danh mục mới" name="ten[]"  class="form-control" OnkeyUp="addText(this,'#alias_<?=$value['code']?>','')"/>
                                            </div>
                                    ...
                                    <?php */ ?>
                                    <div class="form-group">
                                        <input type="hidden" id="alias_<?=$value['code']?>" name="alias[]" value="<?=$row['alias']?>" >
                                    </div>
                            ...
                            <?php if(isset($_GET['id']) and  $_GET['id']!=''){?>
                                <a class="btn btn-warning" href="index.php?p=filter&a=man">Quay lại</a>
                            <?php } ?>
                          
                               <button class="btn btn-primary pull-right" type="submit" name="add_cate"><?php echo (!empty($_GET['id_loai'])) ? 'Cập nhật':'Thêm mới';?></button>
