                     ...
                     <div class="row">
                        <div class="col-md-9">
                            ...
                            <div id="myTabContent" class="tab-content">
                                <?php foreach (get_json('lang') as $key => $value) {
                                if(isset($_GET['id'])){
                                    $row = $d->simple_fetch("select * from #_filter_content where id_code = ".(int)$_GET['id']." and lang = '".$value['code']."' ");
                                }      
                                ?>
                                ...
                                <div role="tabpanel" class="tab-pane fade <?php if($key==0){?> active in<?php }?>" id="<?=$value['code']?>" aria-labelledby="<?=$value['code']?>">
                                    ...
                                    <?php /* ?>
                                     <div class="form-group">
                                        <label class="col-sm-2 control-label">Khoảng giá:</label>
                                        <div class="col-sm-4">
                                            <label>Giá: <span class="text-s_price" style="color: red;"><?php if(isset($_GET['id'])){ echo numberformat($row['s_price']);} ?></span></label>
                                            <input id="s_price" type="text" placeholder="Giá từ >= ?" class="form-control" name="s_price" value="<?= $row['s_price']?>" >
                                        </div>
                                        <div class="col-sm-4">
                                            <label>Giá: <span class="text-e_price" style="color: red;"><?php if(isset($_GET['id'])){ echo numberformat($row['e_price']);} ?></span></label>
                                            <input id="e_price" type="text" placeholder="Đến giá < ?" class="form-control" name="e_price" value="<?= $row['e_price']?>" >
                                        </div>
                                    </div>
                                    <div class="form-group">
                                ...
                                <div class="form-group">
                                    <div class="col-sm-10 col-sm-offset-2">
                                        ...
                                        <button type="submit" name="capnhat" class="btn btn-primary pull-right"><span class="glyphicon glyphicon-floppy-save"></span><?php echo ($_GET['a']=='add') ? 'Thêm mới':'Cập nhật';?></button>
                                        <?php }?>
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="form-group m0 hinh_anh" >
                                ...
                                <span class="box-img2">
                                    ...
                                    <button class="btn btn-xs btn-danger" type="button" onclick="xoa_img('_filter_content','hinh_anh', '<?=$_GET['id']?>','')"><i class="fa fa-trash"></i></button>
                                    <?php }else{ ?>
                            ...
                            <?php 
                            $danhmuc = $d->o_fet("select * from #_filter where lang = '".LANG."' ");
                            ?>
...
<script type="text/javascript">
    ...
    $('#s_price').keyup(function(){
        var num = $(this).val();
        $('.text-s_price').html(formatNumber(num,',','.'));
    });
    $('#e_price').keyup(function(){
        var num = $(this).val();
        $('.text-e_price').html(formatNumber(num,',','.'));
    })
</script>
