...
if(isset($_GET['delete'])){
    $id = (int)$_GET['delete'];
    $d->o_que("delete from #_coupon where id = $id ");
    $d->redirect("index.php?p=coupon&a=man");
}
?>
                ...
                <table class="table table-bordered table-striped table-primary table-hover" id="dataTable1">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span></span><?php echo $value['ho_ten'] ?></td>
                            <td><span>Điện thoại: </span><?php echo $value['sdt']?><br/>
                            <span>Địa chỉ: </span><?php echo $value['address']?><br/>
                            <div>
                                <p>Nội dung: <?=strip_tags($value['content'])?> </p>
                                
                            </div>
                           
                            <td class="text-center" style="width:200px"><?=$value['ngay_hoi']?></td>
                            <td class="text-center" style="width:200px">
                                <?php if($value['trang_thai']==0){ ?>
                                <label class="label label-warning">Chưa xem</label>
                                <?php }else{ ?>
                                 <label class="label label-success">Đả xem</label>
                                <?php } ?>
                            </td>
