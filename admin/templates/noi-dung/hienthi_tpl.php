    ...
    if(!isset($_GET['id_loai'])){
        ...
        if($id_code = $d->insert($data0)) {
            ...
            foreach (get_json('lang') as $key => $value) {
                ...
                $data_cate['background']     =   $background;
        ...
        <div class="row">
            <div class="col-sm-8">
                <div class="box box-primary">
                    ...
                    <?php 
                          //c1
                          $danhmuc_home = $d->o_fet("select * from #_category_noidung where lang='".LANG."' order by id ASC"); 
                    ?>
                                ...
                                <tr>
                                    <td class="text-center">
                                        ...
                                        <button type="button" class="btn btn-xs btn-success" onclick="$('.noi_dun_<?=$value['id']?>').slideToggle();">
                                            <i class="fas fa-arrow-down"></i> Chi tiết (<?=count($hinhanh)?>)
                                        </button>
                                        <?php }?>
                                    ...
                                    <td class="text-center">
                                        <input <?php if($d->checkPermission_edit($id_module)==0){ ?> disabled <?php }?> class="chk_box" type="checkbox" onclick="on_check(this,'#_category_noidung','hien_thi','<?=$value['id']?>')" <?php if($value['hien_thi'] == 1) echo 'checked="checked"'; ?>>
                                    </td>
                                    <td class="text-center">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <form method="POST" action=""  enctype="multipart/form-data">
                            ...
                            <div class="form-group m0 col-sm-6 background" >
                                ...
                                <span class="box-img2">
                                    ...
                                    <img onerror="this.src='img/no-image.png';" src="../img_data/images/<?php echo $row0['background']?>"  id="review_background" alt="NO PHOTO" />
                                    <button class="btn btn-xs btn-danger" type="button" onclick="xoa_img('_category_noidung','background', '<?php echo (!isset($_GET['parent']) ? $_GET['id_loai']:$_GET['id']  )?>','')"><i class="fa fa-trash"></i></button>
                                    <?php }else{ ?>
                                    <img src="img/no-image.png" style="max-width: 100%;max-height: 100%;object-fit: contain;" id="review_background" alt="NO PHOTO" />
                                    <?php }?>
                                    <input type="hidden" value="<?=$row['background']?>" name="background" id="background" class=" form-control">
                                    <a href="filemanager/dialog.php?type=1&field_id=background&relative_url=1&multiple=0" class="btn btn-upload2 iframe-btn" > <i class="fa fa-upload" aria-hidden="true"></i>Chọn ảnh </a>
                                </span>
                            </div>
