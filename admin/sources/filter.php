...
function showdulieu(){
	...
	if($_REQUEST['a'] == 'man'){
            $items = $d->o_fet("select * from #_filter_content where lang ='".LANG."'  order by so_thu_tu asc, id desc");
	}else{
            if(isset($_REQUEST['id'])){
                @$id = addslashes($_REQUEST['id']);
                $items = $d->o_fet("select * from #_filter_content where id_code =  '".$id."' and lang = '".LANG."'");
	...
	if($id != '')
	{
            ...
            if($d->update($data0)) {
                foreach (get_json('lang') as $key => $value) {
                    ...
                    $data['s_price']        =   $d->clear(addslashes($_POST['s_price']));
                    $data['e_price']        =   $d->clear(addslashes($_POST['e_price']));

                    
                    $d->reset();

                    $d->setTable('#_filter_content');
                ...
                $d->redirect("index.php?p=filter&a=man&parent=".$_GET['parent']);
            }else{
                $d->alert("Cập nhật dữ liệu bị lỗi!");
                $d->redirect("Cập nhật dữ liệu bị lỗi", "index.php?p=filter&a=man&parent=".$_GET['parent']);
            }
	}
	else
	{
            ...
            if($id_code = $d->insert($data0)) {
                ...
                $d->redirect("Thêm dữ liệu bị lỗi", "index.php?p=filter&a=man&parent=".$id_loai);
            }
	}
    }else{
        $d->redirect("index.php?p=filter&a=man&parent=".$id_loai);
    }
}
    ...
    if($d->checkPermission_dele($id_module)==1){
        if(isset($_GET['id'])){
            ...
            if($d->delete()){
                ...
                $d->redirect("Xóa dữ liệu bị lỗi", "index.php?p=filter&a=man&parent=".@$_GET['parent']);
            }
        }else {
...
function xoadulieu_mang($id_module){
    ...
    if($d->checkPermission_dele($id_module)==1){
        if(isset($_POST['chk_child'])){
            ...
            $chuoi = trim($chuoi,',');
            if($d->o_que("delete from #_filter_content where id_code in ($chuoi)")){
        ...
        }else $d->redirect("index.php?p=filter&a=man&parent=".@$_GET['parent']);
    }else{
        $d->redirect("index.php?p=filter&a=man&parent=".@$_GET['parent']);
    }
