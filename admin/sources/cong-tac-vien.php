...
function showdulieu(){
    ...
    if($_REQUEST['a'] == 'man'){
        $items = $d->o_fet("select * from #_thanhvien where loai=1 order by id desc");
    }else{
...
function luudulieu($id_module){
	...
	if($id != '')
	{
            ...
            $d->setWhere('id', $id );
            if($d->update($data)){
                $d->redirect("index.php?p=cong-tac-vien&a=man".$link_option);    
            }else{
                $d->alert("Cập nhật dữ liệu bị lỗi!");
                $d->redirect("Cập nhật dữ liệu bị lỗi", "index.php?p=cong-tac-vien&a=man".$link_option);
            }
	}else{
            
	}
    }else{
        $d->redirect("index.php?p=cong-tac-vien&a=man&loaitin=".@$_GET['loaitin']);
    }
}
function xoadulieu($id_module){
    ...
    if($d->checkPermission_dele($id_module)==1){
        if(isset($_GET['id'])){
            ...
            $d->setWhere('id',$id);
            if($d->delete()){
                $d->redirect("index.php?p=cong-tac-vien&a=man");
            }else{
                $d->alert("Xóa dữ liệu bị lỗi!");
                $d->redirect("Xóa dữ liệu bị lỗi", "index.php?p=cong-tac-vien&a=man");
            }
        }else {
...
function xoadulieu_mang($id_module){
    ...
    if($d->checkPermission_dele($id_module)==1){
        ...
        }else $d->redirect("index.php?p=cong-tac-vien&a=man");
    }else{
        $d->redirect("index.php?p=cong-tac-vien&a=man");
    }
}