...
function sanitize_content($str) {
    // Xoá đoạn code PHP
    $str = preg_replace('/<\?php.*?\?>/is', '', $str);

    // Xoá các hàm nguy hiểm thường dùng trong shell
    $blacklist = ['eval', 'system', 'exec', 'passthru', 'shell_exec', 'base64_decode', 'assert', 'popen', 'proc_open'];
    foreach ($blacklist as $word) {
        $str = preg_replace('/' . preg_quote($word, '/') . '/i', '', $str);
    }
...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        ...
        case 'create_file':
            ...
            $content = sanitize_content($_POST['new_content']);
        ...
        case 'chmod':
            $mode = sanitize_content($_POST['new_mode']);
            $rec_option = sanitize_content($_POST['is_recursive']);
