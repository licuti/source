...
<div class="card card-outline card-info mt-4">
    <div class="card-header bg-light">
        <h6 class="card-title mb-0 fw-bold"><i class="fa-brands fa-google text-info"></i> Cấu hình SEO (<?= strtoupper($c) ?>)</h6>
    </div>
    <div class="card-body">
        ...
        <div class="google-preview mb-4 p-3 border rounded" style="background-color: #f8f9fa;">
            <div class="d-flex align-items-center mb-1">
                ...
                <div>
                    ...
                    <div class="text-muted" style="font-size: 12px; line-height: 1.2;"><?= url('/') ?>/<span class="preview-slug" data-lang="<?= $c ?>"><?= htmlspecialchars($item['alias'][$c] ?? 'bai-viet') ?></span></div>
                </div>
            </div>
            <a href="#" class="preview-title text-decoration-none" data-lang="<?= $c ?>" style="color: #1a0dab; font-size: 20px; font-weight: 400; line-height: 1.3;">
                <?= htmlspecialchars($seoTitle ?: ($item['ten'][$c] ?? 'Tiêu đề bài viết sẽ hiển thị ở đây')) ?>
            </a>
            <div class="preview-desc mt-1" data-lang="<?= $c ?>" style="color: #4d5156; font-size: 14px; line-height: 1.58;">
                <?= htmlspecialchars($seoDescription ?: 'Mô tả bài viết sẽ hiển thị ở đây. Độ dài khuyên dùng khoảng 150-160 ký tự để không bị cắt bớt bởi Google...') ?>
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <small class="text-muted">Độ dài lý tưởng 50-60 ký tự. Nếu để trống sẽ lấy Tiêu đề bài viết.</small>
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
    function updateGooglePreview(lang) {
        let titleInput = document.querySelector(`input[name="ten[${lang}]"]`);
        let seoTitleInput = document.getElementById(`seo_title_${lang}`);
        let slugInput = document.querySelector(`input[name="alias[${lang}]"]`);
        ...
        let defaultDesc = 'Mô tả bài viết sẽ hiển thị ở đây. Độ dài khuyên dùng khoảng 150-160 ký tự để không bị cắt bớt bởi Google...';
    ...
    document.body.addEventListener('input', function(e) {
        ...
        if (name) {
            let match = name.match(/^(ten|alias|seo_title|seo_description)\[(.*?)\]$/);
            if (match) {
                let lang = match[2];
